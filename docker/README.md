...
| `multi-verify-partial` | Installs claude-code cleanly, then manually edits the manifest to flip `partial: "agents"` on the entry (with re-computed contentHash); runs `--verify`; asserts the partial-install warning row surfaces with the step name + re-run hint; asserts verify exits non-zero; asserts the other per-file checks still ran (recorded lists are honest under the Phase 0.5 contract) |
| `multi-uninstall-subset` | Installs into 3 harnesses with `--all-detected`, then runs `--uninstall --harness opencode`; asserts opencode's state is removed while claude-code + gemini-cli state is preserved; asserts the manifest is UPDATED (not deleted) with only the remaining entries; asserts the "Preserving global @uluops/cli" notice appears (subset uninstall doesn't touch shared infrastructure); asserts a follow-up `--uninstall` with no filter removes everything cleanly |
| `multi-uninstall-unknown-harness` | Installs claude-code, then runs `--uninstall --harness opencode` (opencode not in manifest); asserts exit non-zero; asserts the error message names the unknown harness AND lists what IS in the manifest so the user can correct; asserts claude-code state is untouched (fail-fast before any work) |
